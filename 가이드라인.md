...
### 4️⃣ `Dockerfile` - 배송 상자 📦
- **역할**: 프로그램을 "포장"해서 어디서든 실행할 수 있게 만들어주는 설명서
- **비유**: 집에서 만든 요리를 배달할 수 있게 상자로 포장하는 것처럼!

### 5️⃣ `k8s-newsletter-cronjob.yaml` - 알람 설정 ⏰
- **역할**: "매일 아침 8시에 자동으로 실행해줘"라고 지시하는 스케줄
- **크론잡(CronJob)**: 정해진 시간에 자동으로 돌아가는 작업
- **설정**: `schedule: "0 23 * * *"` → 매일 한국 시간 8시(UTC 23시)
...
GMAIL_APP_PASSWORD = "xxxx xxxx xxxx xxxx"  # 16자리
RECIPIENT_EMAILS = ["받는이@company.com"]
AZURE_OPENAI_ENDPOINT = "https://xxx.openai.azure.com"
```

**주의사항:**
- App Password는 Google 계정 → 보안 → 앱 비밀번호에서 받으세요!
...
2. ✅ Kubernetes Secrets 사용 (배포 시)
3. ✅ 앱 비밀번호만 사용 (2단계 인증 필수)
